...
- **Exact SRE**: Computes the exact SRE using the HadaMAG algorithm, which leverages Fast Hadamard Transforms (FHT) to reduce the naive $O(4^n)$ complexity down to $O(n2^n)$.
